...
func (e *HiC) loadBodyIndex(key string) (*Body, error) { //loadBodyIndex if it is not in buffer
	...
	go func() {
		...
		for i := int32(0); i < b.NRes; i++ {
			...
			b.Mats[i] = BlockMatrix{unit, resIdx, sumCounts, occupiedCellCount, stdDev, percent95, binSize, blockBinCount, blockColumnCount, blockCount, blockIndexes, make(map[int]*Block), e, int(r), int(c)}
